...
def build_step_wise_generator_output(
    ...
    for traj in trajectory_outputs:
        ...
        assert (
            len(prompt_token_ids_per_turn) == n_turns
            and len(logprobs_per_turn) == n_turns
        ), (
    ...
    if successful_trajectories:
        ...
        rollout_metrics["generate/avg_num_turns"] = sum(
            t.num_turns for t in successful_trajectories
...
class HarborGenerator(GeneratorInterface):
    def __init__(
        ...
        if not agent_kwargs.get("collect_rollout_details", False):
            logger.warning(
                "step_wise_trajectories=true requires collect_rollout_details=true; enabling automatically."
            )
    ...
    async def generate(
        ...
        async def _worker(idx, prompt, trajectory_id):
            result = await self._harbor_agent_loop(
                prompt=prompt, trajectory_id=trajectory_id, cache_salt=cache_salt
            )
