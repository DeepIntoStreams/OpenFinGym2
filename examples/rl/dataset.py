...
class HarborTaskDataset:
    ...
    def _load_data_files(self) -> List[Path]:
        ...
        for data_source in self.data_files:
            ...
            if source_path.is_dir():
                ...
                valid_task_dirs = [
                    d for d in all_dirs if self._is_valid_task_directory(d)
                ]
                ...
                logger.warning(
                    f"File {source_path} cannot be a valid task directory (missing instruction.md)"
                )
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        if index >= len(self.task_paths):
            raise IndexError(
                f"Index {index} out of range for dataset of size {len(self.task_paths)}"
            )
        return {
