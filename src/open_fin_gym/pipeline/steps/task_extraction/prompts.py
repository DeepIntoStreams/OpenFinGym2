...
- A detailed description of an any training datasets
- A detailed description of the test input
- A detailed description of the test output that should be generated
- A detailed description of any test ground-truth data
- A detailed description of the assessment of the task, i.e. what metrics should be used, and how they are applied
...
- Training data:
    ...
    - y_train a [M, 7] target prices following on from x_train
- Test input:
...
- Ground truth data:
    - y_test a [N, 7] matrix of ground-truth prices following on from x_test
- Assessment metrics:
    - MSE: The mean-squared-error between y_pred and y_test

These fields will then be used by downstream tasks to define the task, and write code to retrieve data and assess the task.
...
- The user will be provided with the task description and train and test data (holding out the assessment data). The user will write their
  output to a file, which will then be read and assessed using the task metrics.
...
class PaperTaskSpecification(BaseModel):
    ...
    training_data: list[Dataset] = Field(description="List of training datasets")
    test_input: list[Dataset] = Field(description="List of test input datasets")
    test_output: list[Dataset] = Field(
        description="List of test outputs the user should produce"
    )
    ground_truth_data: list[Dataset] = Field(
        description="List of ground truth datasets used to assess the users output"
    )
