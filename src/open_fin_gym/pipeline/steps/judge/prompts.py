...
- Hugging Face, Kaggle, or well-known academic datasets with clear naming and source context, only if the excerpt gives evidence of a direct, anonymous download path — Kaggle's standard download flow (its API/CLI or "download" button) requires an account and does not qualify on its own; only treat a Kaggle-hosted dataset as public if a direct, unauthenticated file URL is explicitly evidenced.
...
- the dataset requires creating an account, logging in, or obtaining an API key/token to retrieve it programmatically, even if free (e.g. Kaggle's standard flow) — this counts as an interactive-auth barrier regardless of how well-known or "public" the dataset is described as being.
